...
public class LinkedEnhancedPositionalList<E> implements EnhancedPositionalList<E> {
	...
	public Position<E> addBefore(Position<E> p, PositionalList<E> sublist) throws IllegalArgumentException {
		...
		Position<E> returnValue = p;
		Position<E> insertPosition = p;
		Node<E> insertNode = validate( sublist.first() );
		int subSize = sublist.size();

		for ( int i = 0; i < subSize; i++ ){
			addBefore( insertPosition, insertNode.getElement() );
	...
	public Position<E> addAfter(Position<E> p, PositionalList<E> sublist) throws IllegalArgumentException {
		...
		for ( int i = 0; i < subSize; i++ ){
			addAfter( insertPosition, insertNode.getElement() );
			insertNode = insertNode.getNext();
			insertPosition = after( insertPosition );
		}
	...
	public Position<E> addFirst(PositionalList<E> sublist) {
		...
		if ( this.isEmpty() ){
			...
			Node<E> insertNode = validate( sublist.first() );
			Position<E> returnValue = this.first();
			Position<E> insertPosition = this.first();
			int subSize = sublist.size();
	...
	public Position<E> search(E e) {
		Position<E> now = this.first();
		int size = this.size();
		
		for ( int i = 0; i < size; i ++ ){
			if ( now.getElement().equals( e ) ){
				return now;
			}else{
				now = this.after( now );
			}
	...
	public int distance(Position<E> a, Position<E> b) throws IllegalArgumentException {
		...
		Position pa = this.search( a.getElement() );
		Position now = pa;
		int count = 0;	
		
		//in case that b is behind a
		while ( now != null ){
			if ( now != b ){
				count++; 
				now = after( now );
