...
//                new int[]{});
    ...
    public HashMap<String,Object> getBookingDetails(String memberID, int journeyId) throws OlympicsDBException {
    	...
    	try
        {
            Connectin con = getConnection();
            Statement bookingDetails = con.createStatement();
            String query = "SELECT * FROM Booking JOIN Journey USING(journey_id) WHERE booked_for = '" + memberID
                            + "' AND journey_id = " + journeyID;

            ResultSet bookingDetailsResultSet = bookingDetails.executeQuery(query);

            if ( bookingDetailsResultSet.next() )
            {
                booking.put("vehicle", bookingdetailsResultSet.getString(bookingdetailsResultSet.findColumn("vehicle")));
                booking.put("start_day", bookingdetailsResultSet.getString(bookingdetailsResultSet.findColumn("start_date")));
                booking.put("start_time", bookingdetailsResultSet.getString(bookingdetailsResultSet.findColumn("start_time")));
                booking.put("to", bookingdetailsResultSet.getString(bookingdetailsResultSet.findColumn("to")));
                booking.put("from", bookingdetailsResultSet.getString(bookingdetailsResultSet.findColumn("from"))");
                booking.put("booked_by", bookingdetailsResultSet.getString(bookingdetailsResultSet.findColumn("booked_by")));
                booking.put("whenbooked", bookingdetailsResultSet.getString(bookingdetailsResultSet.findColumn("whenbooked")));
            }
